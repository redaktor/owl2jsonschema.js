...
rewritten jsons like property-miltiplicity MUST go to target (options.outputDir), for now they remain in lib
	...
	function createSchema(spec) {
		...
		var makeProperty = function (key) {
			...
			if (rp) {
				...
				if ('maxItems' in rp) setRestriction('maxItems', rp.minItems);
